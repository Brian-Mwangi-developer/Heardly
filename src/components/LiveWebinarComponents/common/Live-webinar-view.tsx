...
import { CTADialogBox } from './CallToActionDialog';
...
export const LiveWebinarView = ({
    ...
    const handleEndStream = async () => {
        ...
        try {
            ...
            call.endCall()
            const res = await changeWebinarStatus.mutateAsync({ webinarId: webinar.id, status: 'ENDED' });
            if (!res.sucess) {
            ...
            toast.error('Failed to end webinar, please try again later');
        } finally {
            setLoading(false);
        }
    }
        ...
        <div className='flex flex-col w-full h-screen max-h-screen overflow-hidden bg-background text-foreground'>
                        ...
                        {isHost && (
                            <div className='flex items-center space-x-1 cursor-pointer'>
                                ...
                                <Button
                                    ...
                                    className='mr-2 bg-purple-500 text-white'
                                >
                                    Go Live
                                </Button>
            ...
            {/* {obsDialogBox && (
                <ObsDialogBox
                    ...
                    rmtpUrl={`rtmps://ingress.stream-io-video.com:443/${process.env.NEXT_PUBLIC_STREAM_API_KEY}.livestream.${webinar.id}`}
                    streamKey={userToken}
                />
            )} */}
        </div>
